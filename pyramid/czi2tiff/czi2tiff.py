...
def main(czifilename, zoomlevel=None):
    ...
    # Check zoom levels
    if zoomlevel not in converter._zoom_levels:
        raise ValueError(usage('Second argument must be valid zoom level: ' + str(converter._zoom_levels)))
    ...
    for channel in range(converter.num_channels()):
        ...
        H, W = map(lambda x: x/zoomlevel, converter.canvas_size())
        ...
        for k in range(K):
            ...
            for j in range(J):
                ...
                output[kstart:kend, jstart:jend] = converter.get_tile_data(channel, zoomlevel, (slice(kstart, kend), slice(jstart, jend)), fill=fill)
    
    filename = czifilename[:-4] + "_" + str(zoomlevel) + "x_zoom.tiff"
    tifffile.imsave(filename, output, bigtiff=True)
...
def usage(mesg):
    return """%s
usage: czi2tiff.py czifile <zoom level>
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            ...
            raise ValueError(usage('%s. First argument must be a CZI file.' % e))
    except:
